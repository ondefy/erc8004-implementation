...
Create a `.env.local` file in the `frontend` directory with your Pinata JWT token:

```
PINATA_JWT=your_pinata_jwt_token
```

This variable is used only in server-side API routes to pin proofs and validation results to IPFS via Pinata.

**Note**: Pinata JWT authentication is required. You can obtain a JWT token from your [Pinata dashboard](https://app.pinata.cloud/developers/api-keys).
